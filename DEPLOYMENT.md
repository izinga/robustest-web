...
- **GoatCounter (self-hosted)** — `goatcounter` systemd
  ...
  third-party IP filters can't interfere. Custom events use flattened
  names (e.g. `contact-form-submitted-partner`).
...
- Plausible (cloud) was used alongside GoatCounter until Aug 2026
  (trial ended); its script has been removed from both layouts.
